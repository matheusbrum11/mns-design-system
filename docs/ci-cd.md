...
| `snapshot.yml` | push em `main`, exceto quando só documentação mudou | Qualidade + publica `X.Y.Z-SNAPSHOT`. A publicação **não reprova o job**: snapshot é conveniência, e uma `main` vermelha por causa dele esconderia falha real de código. Quando falha, sai um aviso com o código HTTP e o que checar. |

> **Merge em `main` que não dispara nada é esperado**, não bug: se o push mexeu
> só em `docs/**` ou em markdown, o `snapshot.yml` é pulado (o artefato não
> mudaria) e o `ci.yml` não tem gatilho de push. Para forçar, use
> *Actions → Snapshot → Run workflow*.
>
> Mudanças em `.github/**` **disparam** o snapshot de propósito: mexer na
> esteira precisa exercitar a esteira.
